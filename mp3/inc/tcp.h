...
class tcp_server : tcp_server_intf {
public:
...
public:
     ...
     */
	virtual int setup_connection(string host, string port) = 0;
    /**
     * reads the specified number of bytes from the given socket
     * returns number of bytes read, 0 on socket disconnect, and -1 on failure
     */
...
class tcp_client : tcp_client_intf {
public:
	int setup_connection(string host, string port);
