...
testing::register_test inserting_small("member_list.inserting_small",
    "Tests inserting / reading member lists with less than 5 members", [] (logger *lg)
{
...
testing::register_test inserting_large("member_list.inserting_large",
    "Tests inserting / reading / sortedness of member list with more than 5 members", [] (logger *lg)
{
    member_list ml("local", lg);
    std::vector<int> ids;
    for (int i = 1; i < 11; i++) {
...
testing::register_test removing("member_list.removing_large",
    "Tests removing from member list, while verifying sortedness and neighbors", [] (logger *lg)
{
    member_list ml("8", lg);
    std::vector<int> ids;
    for (int i = 1; i < 8; i++) {
