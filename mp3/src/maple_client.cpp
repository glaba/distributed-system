...
bool maple_client_impl::run_job(string maple_node, string local_maple_exe, string maple_exe, int num_maples,
    string sdfs_intermediate_filename_prefix, string sdfs_src_dir)
{
    do {
        sdfsc->put_operation(local_maple_exe, maple_exe);

        maple_message msg(0, maple_start_job{maple_exe, num_maples, sdfs_intermediate_filename_prefix, sdfs_src_dir});

        // Send the data to the node
        std::unique_ptr<tcp_client> client = fac->get_tcp_client();
        int fd = client->setup_connection(maple_node, config->get_maple_master_port());
        if (fd < 0 || client->write_to_server(fd, msg.serialize()) <= 0) {
            error = "Node at " + maple_node + " is not running Maplejuice, retry";
            return false;
        }
        ...
        if (response_msg.get_msg_type() == maple_message::maple_msg_type::NOT_MASTER) {
            ...
            if (master_node == "") {
                ...
                return run_job(master_node, local_maple_exe, maple_exe, num_maples, sdfs_intermediate_filename_prefix, sdfs_src_dir);
            }
