...
int main(int argc, char **argv) {
    ...
    cli_parser.add_option<string>("i", "introducer", "The hostname of a node already in the group, or none if we are the first member", &introducer);
    ...
    cli_parser.add_option<int>("sip", "port", "The TCP port used for communication between nodes in SDFS (default 1234)", &sdfs_internal_port);
    cli_parser.add_option<int>("smp", "port", "The TCP port used for communication between clients and the master node in SDFS (default 1235)", &sdfs_master_port);
    cli_parser.add_option<int>("mip", "port", "The TCP port used for communication with Maple nodes (default 1236)", &maple_internal_port);
    cli_parser.add_option<int>("mmp", "port", "The TCP port used for communication with the Maple master (default 1237)", &maple_master_port);
